...
cox_based_train <- function(p,c,alpha,
                      data_fit,
                      data_calib,
                      type,
                      dist,
  ...
  if(type == "quantile"){
    ...
    ret = list(score = score, mdl = mdl, weight_calib = weight_calib, bw = NULL)
    return(ret)
    }
  ...
  if(type == "percentile"){
    ...
    ret = list(score = score, weight_calib = weight_calib, bw = bw, new_quant = NULL)
    return(ret)
    }
  return("Error: type must be either 'quantile' or 'percentile'")
}


cox_based_predict <- function(x,c,alpha,
                      type,
                      weight_calib,
                      weight_new,
                      score,
