...
data = read.csv("HF_Project/final_df_June11.csv")
head(data)

# Convert columns after censoring_time into true/false
data[, -(1:22)] <- lapply(data[, -(1:22)], as.factor)

other_variables = colnames(data)[23:136][-11]
all_variables = c("insurance", "language", "marital_status", "los", "gender", "anchor_age", "race", "first_careunit", "last_careunit", other_variables)
all_variables
...
X[, "first_careunit"] = as.factor(X[, "first_careunit"])
X[, "last_careunit"] = as.factor(X[, "last_careunit"])
censored_T = data$censored_T
C = data$censoring_time
#C = 365 - X[, "los"] - X[, "anchor_age"] - runif(n, 0, 100)
#print(summary(C))
#C = runif(n, 100, 365)
censored_T = pmin(data$censored_T, C)
event = censored_T < C #die before censoring time
...
write.csv(output_table, paste0("HF_Project/output_July18/July18_output_large_alpha_001_seed_", seed, ".csv"), row.names=FALSE)

#write.csv(predicted_lbd, "June21_predicted_lbd.csv", row.names=FALSE)
#write.csv(predicted_lbd_cox, "June21_predicted_lbd_cox.csv", row.names=FALSE)
#write.csv(cbind(cox_cov,our_cov), "June21_coverage.csv", row.names=FALSE)
